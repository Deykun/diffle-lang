...
const AboutLanguageNeighbours = ({
  data: {
    spellchecker: {
      accepted: { all },
      substrings: { first, middle, last },
    },
  },
}: Props) => {
  const { firstKeys, middleKeys, lastKeys } = useMemo(() => {
    return {
      firstKeys: {
            ...
            <div key={key}>
              <ul className="about-language-neighbours-list--end">
                {firstKeys[key].map((part) => (
                  <li key={part} className="has-tooltip">
                    {capitalize(part)}
                  ...
                  >
                    ...
                    <span
                      ...
                      dangerouslySetInnerHTML={{
                        __html: t('statistics.languageDescriptionHighestLetterForCommon', {
                          // It works fine here
                          maxletter: `"${part}"`,
