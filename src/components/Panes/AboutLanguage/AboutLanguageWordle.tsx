...
const AboutLanguageWordle = ({ data }: Props) => {
  const {
    spellchecker: {
      accepted: { allWordleWords },
      letters: { wordle: letterPositions },
      wordle,
    },
    meta,
  } = data;
  ...
  const {
    word: firstWord,
    result: { green: firsrtWordCorrect, orange: firsrtWordPosition, gray: firsrtWordIncorrect },
  } = wordsData[0];
  ...
  return (
    <section className="about-language-wordle">
      ...
      <div className="about-language-wordle-best-word-statistics">
        ...
        >
          <span>{(100 * (firsrtWordCorrect / (allWordleWords * 5))).toFixed(2)}%</span>
        ...
        >
          <span>{(100 * (firsrtWordPosition / (allWordleWords * 5))).toFixed(2)}%</span>
        ...
        >
          <span>{(100 * (firsrtWordIncorrect / (allWordleWords * 5))).toFixed(2)}%</span>
        ...
        <ul className="about-language-wordle-positions-list">
          {[1, 2, 3, 4, 5].map((position, index) => (
            <li key={position}>
              ...
              {Object.entries(letterPositions[index])
                .slice(0, 5)
                .map(([letter, value]) => (
                  <strong
                    key={`${letter}-${value}`}
                  ...
                  >
                    ...
                    <span className="tooltip">
                      <strong className="text-uppercase">{letter.replace('ß', 'ẞ')}</strong> {' - '}
