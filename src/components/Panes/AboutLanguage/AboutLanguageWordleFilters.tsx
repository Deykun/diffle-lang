...
const AboutLanguageWordleFilters = ({ filterGroupBy, setFilterGroupBy }: Props) => {
  ...
  return (
    <details className="about-language-wordle">
      ...
      <div className="details-content">
        ...
        <nav>
          {[BestWordleType.InWords, BestWordleType.LetterPosition, BestWordleType.UniqueLetterPosition].map(
            (filter) => (
              <Button
                key={filter}
        ...
        <nav>
          {[BestWordleType.BestGreen1_5, BestWordleType.BestGreen2_0].map((filter) => (
            <Button
              key={filter}
              onClick={() => setFilterGroupBy(filter)}
              isInverted
