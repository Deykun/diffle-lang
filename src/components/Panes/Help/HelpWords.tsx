...
const HelpWords = ({ helpGuesses, tEnd = '' }: Props) => {
  ...
  const {
    ...
    lines.firstAndLast = {
      message: 'help.firstAndLast',
      values: {
        first: firstAffix?.text?.toLocaleUpperCase() || '',
        last: lastAffix?.text?.toLocaleUpperCase() || '',
      },
    };
      ...
      <>
          ...
          <p>
              <span
                ...
                dangerouslySetInnerHTML={{ __html: t(order1.message, order1.values) }}
              />
              <br />
              <span
                // eslint-disable-next-line react/no-danger
                dangerouslySetInnerHTML={{ __html: t(order2.message, order2.values) }}
              />
          </p>
          ...
          <p
            // eslint-disable-next-line react/no-danger
            dangerouslySetInnerHTML={{ __html: t(inRow.message, inRow.values) }}
          />
          )}
          ...
          <p
            // eslint-disable-next-line react/no-danger
            dangerouslySetInnerHTML={{ __html: t(firstAndLast.message, firstAndLast.values) }}
          />
          )}
