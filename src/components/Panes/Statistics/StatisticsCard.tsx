...
type StatisticForCard = StatisticDataForCard &
  Filters & {
    wonStreak?: number;
    lostStreak?: number;
    bestStreak?: number;
      ...
      <div id="sharable-card" className={clsx('statistics-card-wrapper')}>
        <div className="statistics-card">
          ...
          <div className="circles-below-visual">
            <p className={clsx('statistics-letters')}>
              <strong>
                ...
                <CircleScale
                  breakPoints={BREAKPOINTS.LETTERS}
                  startFrom={START_FROM.LETTERS}
                  value={lettersPerGame}
                  isInvert
                  isScaleOnLeft
                />
              </strong>
            ...
            <p className={clsx('statistics-words')}>
              ...
              <strong>
                ...
                <CircleScale
                  breakPoints={BREAKPOINTS.WORDS}
                  startFrom={START_FROM.WORDS}
                  value={wordsPerGame}
                  isInvert
                />
              </strong>
          ...
          <p className={clsx('statistics-letters', 'has-tooltip', 'has-tooltip-from-left')}>
            <strong>{lettersPerGame.toFixed(1)}</strong>
            <span>{t('statistics.letters')}</span>
          ...
          <p className={clsx('statistics-words', 'has-tooltip', 'has-tooltip-from-right')}>
            <span>{t('statistics.medianWordsBefore')}</span>
            <strong>{wordsPerGame.toFixed(1)}</strong>
            <span>{t('statistics.medianWords')}</span>
            <span className="tooltip">
              {t('statistics.median')}
              {' ('}
          ...
          <div className="statistics-text">
            <p>
              {t('statistics.averageLetters')} <strong>{lettersPerWord.toFixed(1)}</strong>{' '}
              {t('statistics.inWord')}
            </p>
            ...
            <p className="has-tooltip tooltip-relative">
              ...
              {timePerGame.hours > 0 && (
                <>
                  <strong>{timePerGame.hours}</strong>h
                </>
              )}{' '}
              {timePerGame.minutes > 0 && (
                <>
                  <strong>{timePerGame.minutes}</strong>m
                </>
              )}{' '}
              {timePerGame.seconds > 0 && (
                <>
                  <strong>{timePerGame.seconds}</strong>s
                </>
              )}
              <span className="tooltip">
                ...
                {totalTime.hours > 0 && (
                  <>
                    <strong>{totalTime.hours}</strong> h
                  </>
                )}{' '}
                {totalTime.minutes > 0 && (
                  <>
                    <strong>{totalTime.minutes}</strong> m
                  </>
                )}{' '}
                {totalTime.seconds > 0 && (
                  <>
                    <strong>{totalTime.seconds}</strong> s
                  </>
                )}
              </span>
            </p>
          ...
          <p className="statistics-letters-types">
            <span className="correct has-tooltip has-tooltip-from-left">
              <strong>
                ...
                <CircleScale
                  breakPoints={BREAKPOINTS.LETTER_TYPES}
                  value={lettersCorrect * 100}
                  isPercentage
                />
              </strong>
            ...
            <span className="incorrect typed has-tooltip has-tooltip-from-right">
              <strong>
                ...
                <CircleScale
                  breakPoints={BREAKPOINTS.LETTER_TYPES}
                  value={lettersTypedKnownIncorrect * 100}
                  isPercentage
                />
              </strong>
          ...
          <div className="statistics-other">
            ...
            <p className="statistics-other-percentage">
              <strong>
                {((totalWon / totalGames) * 100).toFixed(0)}
                <small>%</small>
              </strong>
            ...
            {canShowStreak && bestStreak > 0 && (
              <p className="has-tooltip has-tooltip-from-right tooltip-relative">
                <strong>{bestStreak}</strong> {t('statistics.totalBestStreak')}
                <span className="tooltip">
                  {t('statistics.streakTooltipWithWorstStreak')} <strong>{worstStreak}</strong>
                </span>
              </p>
            )}
          </div>
          <footer>
            ...
            <span className="statistics-url">{diffleURL}</span>{' '}
            <StatisticsCardActiveFilters
