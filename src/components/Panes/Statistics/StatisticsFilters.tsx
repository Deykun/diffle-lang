...
const StatisticsFilters = ({ setFiltersData }: Props) => {
  ...
  const {
    paneParams: { modeFilter: paneModeFilter = '' },
  } = usePanes();
  const initialModeFilter =
    keepIfInEnum<ModeFilter>(paneModeFilter, ModeFilter) ?? INITIAL_FILTERS.modeFilter;
  ...
  const [charactersFilter, setModeCharactersFilter] = useState<CharactersFilter>(
  ...
  useEffect(() => {
    ...
    dispatch(
      track({
        name: 'click_statistics_filter',
        params: { filters: `${modeFilter}_${charactersFilter}_${lengthFilter}` },
      }),
    );
  ...
  useEffect(() => {
    ...
    const shouldResetSpecialCharacters =
      !hasLanguageSpecialCharacters && charactersFilter !== CharactersFilter.All;
