...
const { resultsByUser, resultsByLang } = Object.values(resultsBySource)
  .flatMap((v) => v)
  .reduce(
    (
      stack: {
...
const usersSummary = Object.entries(resultsByUser).reduce(
  (stack, [lang, resultsByNickForLang]) => {
    stack[lang] = Object.keys(resultsByNickForLang).reduce((stack, nick) => {
      stack[nick] = getInfoAboutResults(
        Object.values(resultsByNickForLang[nick])
      );
